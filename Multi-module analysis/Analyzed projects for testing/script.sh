...
echo "Preparing the necessary POM files to build the submodules..."
...
# run analysis
echo "Building and then running the analysis..."
mvn clean install --fail-never sonar:sonar -Dsonar.token="$sonarToken"	# builds all the submodules (skipping the ones that fail) and then launches the analysis
